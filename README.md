...
# Development server
Run `npm start`/`npm run start` in the project directory to run a webpack server. Each change in the game's code should reload the webpage (except for mods). Use `Ctrl+C` to stop the server.
